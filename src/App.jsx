...
// ── Semana epidemiológica estándar CDC / MINSAL Chile ──────────────────────
...
// ── Establecimientos SSMC por Polo ─────────────────────────────────────────
...
// Paleta Institucional SSMC / MINSAL
const P = {
  azul:       "#1A3A6B",
  azulDark:   "#0F2347",
  azulLight:  "#EEF2F8",
...
export default function App() {
  ...
  const handleSubmit = () => {
    if (!form.fecha || !form.establecimiento || !form.demanda_total) {
      showToast("Por favor completa fecha, establecimiento y demanda", "error");
      return;
    }
  ...
  return (
    <div style={{ background: P.bg, minHeight: "100vh", fontFamily: "sans-serif", color: P.text }}>
      {toast && (
        <div style={{
          position: "fixed", top: 20, right: 20, zIndex: 9999,
          background: toast.type === "error" ? P.rojo : P.verde,
      ...
      <div style={{ background: P.azul, borderBottom: `3px solid ${P.rojo}` }}>
        <div style={{ padding: "12px 24px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div>
            <h1 style={{ color: "#FFF", fontSize: 16, fontWeight: 700 }}>Sistema de Monitoreo de Urgencias APS — SSMC</h1>
            <p style={{ color: "rgba(255,255,255,0.7)", fontSize: 11 }}>Servicio de Salud Metropolitano Central · Ministerio de Salud</p>
          </div>
          <div style={{ display: "flex", gap: 10 }}>
            <button onClick={exportExcel} style={{ background: "transparent", border: "1px solid rgba(255,255,255,0.4)", color: "#FFF", padding: "6px 12px", borderRadius: 6, cursor: "pointer", fontSize: 12 }}>Exportar Excel</button>
            <button onClick={() => setMostrarPDF(true)} style={{ background: P.rojo, border: "none", color: "#FFF", padding: "6px 12px", borderRadius: 6, cursor: "pointer", fontSize: 12, fontWeight: 700 }}>Reporte PDF</button>
          </div>
        </div>
          ...
          {[
            ...
            <button
              ...
              style={{
                ...
                border: "none", borderBottom: tab === t.id ? `3px solid ${P.rojo}` : "3px solid transparent",
      ...
      <main style={{ padding: 24, maxWidth: 1200, margin: "0 auto" }}>
        {tab === "dashboard" && (
          <div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(140px, 1fr))", gap: 12, marginBottom: 20 }}>
              {[
                { label: "Demanda Total", val: kpis.demanda.toLocaleString("es-CL"), color: P.azul },
                { label: "Atendidos", val: kpis.atendidos.toLocaleString("es-CL"), color: P.verde },
                { label: "Respiratorias", val: kpis.respiratorias.toLocaleString("es-CL"), color: P.amber },
                { label: "Abandonos", val: kpis.abandonos.toLocaleString("es-CL"), color: P.rojo },
                { label: "T° Espera Prom.", val: `${kpis.promEspera} min`, color: "#0284C7" },
              ].map(k => (
                <div key={k.label} style={{ background: "#FFF", border: `1px solid ${P.border}`, borderLeft: `4px solid ${k.color}`, borderRadius: 8, padding: 14 }}>
            ...
            <div style={{ background: "#FFF", border: `1px solid ${P.border}`, borderRadius: 8, padding: 20 }}>
              <h3 style={{ fontSize: 14, fontWeight: 800, color: P.azulDark, marginBottom: 14 }}>Comportamiento Diario de Atenciones</h3>
              <ResponsiveContainer width="100%" height={260}>
                <BarChart data={dataXDia}>
                  <CartesianGrid strokeDasharray="3 3" />
                  <XAxis dataKey="dia" tick={{ fontSize: 10 }} />
                  <YAxis tick={{ fontSize: 11 }} />
                  <Tooltip />
                  <Bar dataKey="demanda" name="Demanda" fill={P.azul} />
                  <Bar dataKey="atendidos" name="Atendidos" fill={P.verde} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </div>
        ...
        {tab === "formulario" && (
          <div style={{ background: "#FFF", border: `1px solid ${P.border}`, borderRadius: 8, padding: 24, maxWidth: 600, margin: "0 auto" }}>
            <h2 style={{ fontSize: 16, fontWeight: 800, color: P.azulDark, marginBottom: 16 }}>{editId ? "Editar Registro" : "Ingresar Atenciones"}</h2>
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              <div>
                <label style={{ fontSize: 11, fontWeight: 700, color: P.muted }}>Fecha</label>
              ...
              <div>
                <label style={{ fontSize: 11, fontWeight: 700, color: P.muted }}>Establecimiento</label>
              ...
              <div>
                <label style={{ fontSize: 11, fontWeight: 700, color: P.muted }}>Pacientes Atendidos</label>
              ...
              <div>
                <label style={{ fontSize: 11, fontWeight: 700, color: P.muted }}>Atenciones Respiratorias</label>
              ...
              <button onClick={handleSubmit} style={{ background: P.azul, color: "#FFF", border: "none", padding: 10, borderRadius: 6, fontWeight: 700, cursor: "pointer", marginTop: 10 }}>Guardar Registro</button>
            </div>
          </div>
        )}
        ...
        {tab === "tabla" && (
          <div style={{ background: "#FFF", border: `1px solid ${P.border}`, borderRadius: 8, padding: 20 }}>
            <h2 style={{ fontSize: 16, fontWeight: 800, color: P.azulDark, marginBottom: 14 }}>Tabla de Registros SSMC</h2>
                ...
                {filtrados.map(r => (
                  <tr key={r.id} style={{ borderBottom: `1px solid ${P.border}` }}>
                    ...
                    <td style={{ padding: 8, textAlign: "center" }}>
                      <button onClick={() => handleEdit(r)} style={{ marginRight: 8, border: "none", background: "transparent", color: P.azul, cursor: "pointer" }}>Editar</button>
                      <button onClick={() => handleDelete(r.id)} style={{ border: "none", background: "transparent", color: P.rojo, cursor: "pointer" }}>Eliminar</button>
                    </td>
                  </tr>
                ))}
              </tbody>
